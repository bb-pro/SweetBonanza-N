...
class ShapeValidator {
    ...
    func validatePath(for level: Int, drawnPath: UIBezierPath, referenceFrame: CGRect) -> Bool {
        ...
        case 10:
            return hasTwoRectangles(drawnPath, in: referenceFrame) // NEW CASE
        case 11:
            return hasFiveCircles(drawnPath, in: referenceFrame) // NEW CASE
        default:
            return false
        }
    }
...
//        let averageAngle = angles.reduce(0, +) / CGFloat(angles.count)
    ...
    private func hasTwoRectangles(_ path: UIBezierPath, in frame: CGRect) -> Bool {
        return true
    }

    private func hasFiveCircles(_ path: UIBezierPath, in frame: CGRect) -> Bool {
        let tolerance: CGFloat = 5.0  // Small tolerance for variations
        var circleCount = 0

        // Define a slightly smaller frame to match the circles within tolerance
        let adjustedFrame = frame.insetBy(dx: tolerance, dy: tolerance)
        
        // Iterate through subpaths to count the circular shapes
        path.cgPath.applyWithBlock { elementPointer in
            let bounds = path.bounds
            
            // Check if the subpath is approximately circular
            if bounds.intersects(adjustedFrame),
               abs(bounds.width - bounds.height) < tolerance {
                circleCount += 1
            }
        }

        // Return true if exactly 5 circular shapes are detected
